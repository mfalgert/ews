...
def parse_flags():
    ...
    parser.add_argument("--dst_ips", nargs="+", default=["10.0.11.10"])
    parser.add_argument("--ews_ip", default="10.0.1.10")
    parser.add_argument("--routers", nargs="+", default=["10.0.1.10", "10.0.2.10", "10.0.3.10", "10.0.4.10", "10.0.5.10", "10.0.6.10", "10.0.7.10", "10.0.8.10", "10.0.9.10", "10.0.10.10", "10.0.11.10"])
    parser.add_argument("--ews_asn", default="100")
    parser.add_argument("--used_ases", nargs="+", default=["100", "200", "300", "400", "500", "600", "700", "800", "900", "1000", "1100"])
    parser.add_argument("--pass_time", default=10)
            ...
            if entry["path"] in new_path:
                if entry["timestamp"] >= convergence_start and new_ts >= entry["timestamp"]:
                        if entry["action"] == "announced":
                            ...
                            new_ases_copy = list(common_ases)
...
def learning_process(prefix, convergence_start, router_paths_cfg, curr_path, prev_path, updates_received):
    ...
    if prev_path != curr_path:
        ...
        if final_update:
            ...
            common_asn = 0
            common_ases = [FLAGS.ews_asn]
            common_ases_copy = list(common_ases)
            early_updates[FLAGS.ews_asn] = {"update": final_update, "ases": common_ases_copy}
            ...
            for i, asn in enumerate(prev_path_split[:-1]):
                ...
                updates_list = updates_received[asn]
                for entry in updates_list:
                    entry_check = " ".join(entry["path"].split()[:2])
                    if entry_check in prev_path:
                        if entry["action"] == "withdrawn" and asn == "300":
                            print "yeee 1: " + str(entry["timestamp"]) + " / conv.start: " + str(convergence_start) + " / prev_ts: " + str(prev_ts)
                        if entry["timestamp"] >= convergence_start and entry["timestamp"] <= prev_ts:
                            ...
                            if entry["action"] == "announced":
                                new_path_split = entry["path"].split()[1:]
            ...
            if early_updates:
                for asn, updt in early_updates.iteritems():
                    ...
                    for asn_entry in updt["ases"]:
                        configs += asn_entry + "/"
                        for ip, dicts in updt["update"]["config"][asn_entry].iteritems():
        ...
        if prefix in last_decision:
            ...
            if type == "EBGP":
                ...
                if prefix in last_updates and asn in last_updates[prefix]:
                    router_paths_copy = copy.deepcopy(router_paths[prefix])
                    update_dict = {"timestamp": timestamp, "action": action, "path": path, "config": router_paths_copy}
                    ...
                    threading.Thread(target=decision_process, args=[router_paths_copy, prefix, asn, action, path]).start()
